...
6. **Write design doc** — save to the repo's planning-docs location if CLAUDE.md/AGENTS.md defines one (e.g. `notes/project-planning/<feature>/YYYY-MM-DD-<topic>-design.md`), else `docs/superpowers/specs/YYYY-MM-DD-<topic>-design.md`, and commit
...
- Before proposing any work item, grep the project's backlog/todo files for its key nouns; an item already filed carries prior decisions you must read
...
One override: a question whose nouns name product objects the user has never seen — things this feature would _create_ — is a visual question even when it reads as conceptual. See the guide's "Ground in the Real Product" section.
